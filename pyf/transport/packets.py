...
class Packet(object):
    ...
    def itervalues(self):
        return map(operator.itemgetter('value'), iter(self.__data.values()))
...
#                     for key, vd in data.items()])
    ...
    def __setattr__(self, attribute, value):
        if '_Packet__initialised' not in self.__dict__:
    ...
    def unserialize_value(self, value, metadata):
        ...
        if metatype == 'int' or metatype == 'string':
            ...
            value = datetime.datetime(*list(map(int, re.split('[^\d]', value)[:-1])))
        elif metatype == 'decimal':
    ...
    def serialized(self):
        output_dict = dict()
        for key, key_dict in self.__data.items():
